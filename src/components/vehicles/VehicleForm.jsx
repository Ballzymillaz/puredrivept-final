...
export default function VehicleForm({ vehicle, onSubmit, isLoading, drivers }) {
  ...
  const handlePhotoUpload = async (e) => {
    ...
    setForm(s => ({ ...s, photo_url: file_url }));
        ...
        <div className="flex items-center gap-3">
          {form.photo_url ? (
            <div className="relative">
              <img src={form.photo_url} alt="Veículo" className="w-28 h-20 object-cover rounded-lg border border-gray-200" />
              <button type="button" onClick={() => handleChange('photo_url', '')} className="absolute -top-1.5 -right-1.5 w-5 h-5 bg-red-500 rounded-full flex items-center justify-center hover:bg-red-600">
                <X className="w-3 h-3 text-white" />
              </button>
            </div>
          ) : (
            <label className="flex items-center gap-2 px-4 py-2 border border-dashed border-gray-300 rounded-lg cursor-pointer hover:border-blue-400 text-sm text-gray-500 hover:text-blue-600 transition-colors">
