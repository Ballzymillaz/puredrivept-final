...
export default function RelatoriosFrota() {
  const [filters, setFilters] = useState({ fleet_manager_id: '', date_from: '', date_to: '' });
  ...
  const filteredDrivers = useMemo(() => {
    if (!filters.fleet_manager_id) return drivers;
    return drivers.filter(d => d.fleet_manager_id === filters.fleet_manager_id);
  }, [drivers, filters.fleet_manager_id]);
  ...
  const stats = useMemo(() => {
    ...
    return { totalGross, totalNet, totalDeductions, activeDrivers, avgGrossPerDriver, perDriver, chartData, byContract, totalWeeks };
    ...
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-xl border p-4 grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="space-y-1">
          ...
          <Select value={filters.fleet_manager_id || 'all'} onValueChange={v => setFilter('fleet_manager_id', v === 'all' ? '' : v)}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <Card className="lg:col-span-2">
          <CardHeader className="pb-2"><CardTitle className="text-sm font-semibold">Top 10 — Rendimento Bruto vs Líquido</CardTitle></CardHeader>
          <CardContent>
            {stats.chartData.length > 0 ? (
              ...
              <div className="h-[260px] flex items-center justify-center text-sm text-gray-400">Sem dados para o período selecionado</div>
            )}
          </CardContent>
        </Card>
